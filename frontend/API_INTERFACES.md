...
所有接口在开发阶段将使用Mock数据，Mock实现位置：
- `lib/api/mock.ts`
- 每个API调用都会标注 `// MOCK: 临时Mock实现，需替换为真实API`
...
### 新增页面
- `/wallet` - 钱包管理页面
- `/my-datasets` - 我的数据集管理（商家专用）
- `/transactions` - 交易记录页面
- `/favorites` - 收藏夹页面
- `/dashboard` - 用户仪表板页面

### 新增组件
- `WalletAlert` - 钱包状态提醒组件
- `PurchaseModal` - 购买确认弹窗
- `DownloadProgress` - 下载进度组件
- `ReviewSystem` - 评价系统组件
- `useFavorites` - 收藏管理Hook
- `useDataset` - 数据集管理Hook
- `useBlockchain` - 区块链交互Hook
...
- `/admin/users` - 用户管理系统
- `/admin/datasets` - 数据集管理系统
- `/admin/fingerprint` - 指纹检测系统

### 新增高级搜索组件
- `AdvancedSearchBar` - 高级搜索栏组件
- `AdvancedFilterSidebar` - 高级筛选侧边栏组件
...
- 搜索历史记录
- 热门搜索推荐
- 多维度筛选（分类、价格、文件大小、格式、评分、下载量、时间）
- 高级搜索选项（搜索范围、精确匹配、排除词汇）
...
## 第四阶段新增功能总结 (Phase 4) - 钱包绑定限制系统

### 新增钱包管理功能
- 钱包地址绑定限制：用户绑定钱包后不能随意更换
- 管理员审核系统：钱包更换需要管理员批准
- 地址一致性验证：区块链操作前验证钱包地址一致性
- 用户友好提示：更换钱包的风险和后果说明

### 新增组件
- `WalletChangeRequest` - 钱包更换申请组件
- `WalletValidation` - 钱包地址验证组件
- `WalletAlert` - 钱包状态警告组件

### 新增管理员功能
- `/admin/wallets` - 钱包管理页面
- 钱包更换请求审批
- 用户钱包绑定状态监控
- 钱包更换历史记录
